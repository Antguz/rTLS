  ...
  if(class(cloud) == "neighborhood") {  ####For objects of class "neighborhood"
    ...
    if(is.null(cloud_b) == TRUE) {
      ...
      if(method == "distance") {  #Method distance
        ...
        results <- adply(cloud, .margins = 1, .fun = dist_dimensionality, cloud = cloud, radius = radius, .progress = "text", .parallel = par, .paropts = pack, .inform = FALSE)
      ...
      if(method == "distance") {  #Method distance
        pack <- list(.packages = c("dplyr", "bio3d"))
        results <- adply(cloud, .margins = 1, .fun = dist_dimensionality, cloud = cloud_b, radius = radius, .progress = "text", .parallel = par, .paropts = pack, .inform = FALSE)
