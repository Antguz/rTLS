...
#'
#' @details The function estimate 10 main statistics of the voxels. Specifically, the first four columns (ei. \code{Voxel.size}, \code{N_voxels}, \code{Volumen}) describe the size of the voxel used, the number of voxels created, the total volumen that they represent, and the surface area that they cover.
...
summary_voxels <- function(voxels, voxel.size = NULL, bootstrap = FALSE, R = NULL) {
  ...
  if(bootstrap == FALSE ) {
      frame <- data.table(Voxel.size, N_voxels, Volumen, Surface, Density_mean, Density_sd, H, Hmax, Equitavility, Negentropy)
    ...
    frame <- data.table(Voxel.size, N_voxels, Volumen, Surface, Density_mean, Density_sd, H, Hmax, Equitavility, Negentropy, H_boot_mean, H_boot_sd, Equitavility_boot, Negentropy_boot)
  }
  return(frame)
}
