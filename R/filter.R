...
#' @param build A \codecharacter} describing the search structure to be used: \code{"kdtree"}, \code{"kmeans"}, \code{"linear"}. Default \code{"kdtree"}.
...
filter <- function(cloud, method, radius, min_neighbours, k, nSigma, edge_length, build = "kdtree", threads = 1L, checks = 20) {
  ...
  if(method == "SOR") {
    ...
    point <- knn(cloud, cloud, (k+1), same = TRUE, build, threads = threads, checks = checks)
  ...
  if(method == "min_neighbors") {
    ...
    neighbors <- radius_search(cloud, cloud, radius, max_neighbour = (min_neighbours+1), same = TRUE, build, threads = threads, checks = checks)
  ...
  if(method == "voxel_center") {
    ...
    point <- knn(center[, 1:3], cloud, k = 1, same = FALSE, build, threads = threads, checks = checks)
