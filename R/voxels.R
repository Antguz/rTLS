...
#'
#' @param cloud A \code{data.table} with *XYZ* coordinates in the first
#' three columns.
#' @param voxel.size A positive \code{numeric} vector with the voxel edge
#' length. It use the same dimensional scale of the point cloud.
#' @param threads An \code{integer} specifying the number of threads to use
#' for parallel processing. Experiment to see what works best for your data
#' on your hardware.
#' @param obj.voxels Logical. If \code{obj.voxel = TRUE}, it returns an object
#' of class \code{"voxels"}, If \code{obj.voxel = FALSE}, it returns a
#' \code{data.table} with the coordinates of the voxels created and the
#' number of points in each voxel. \code{TRUE} as default.
#'
#' @details Voxels are created from the negative to the positive *XYZ* coordinates.
#'
#' @return If \code{obj.voxels = TRUE}, it return an object of class
#' \code{"voxels"} which contain a list with the points used to create the voxels,
#' the parameter \code{voxel.size}, and the \code{voxels} created.
#' If \code{FALSE}, it returns a \code{data.table} with the coordinates of the
#' voxels created and the number of points in each voxel.
#'
...
voxels <- function(cloud, voxel.size, threads = 1L, obj.voxels = TRUE) {
  ...
  vox <- as.data.table(voxelization_rcpp(as.matrix(cloud), voxel.size, threads))
