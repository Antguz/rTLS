...
#' @param edge.length A positive \code{numeric} vector with the voxel-edge length for the x, y, and z coordinates. It use the same dimensional scale of the point cloud.
...
#' voxels(pc_tree, edge.length = c(0.5, 0.5, 0.5))
#'
#'
#' @export
voxels <- function(cloud, edge.length, threads = 1L, obj.voxels = TRUE) {

  vox <- as.data.table(voxelization_rcpp(as.matrix(cloud), edge.length, threads))
